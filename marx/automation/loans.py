...
- Identificar los préstamos y deudas abiertas y cerradas a cierta fecha.
- Generar un default en un préstamo o deuda. Esto es, se asume que el préstamo
no será devuelto o la deuda saldada. Se marcará colocando '!' al principio de
la etiqueta.
...
class Loan:
    ...
    def __str__(self) -> str:
        pos = "Deudor" if self.position == Loan.DEBTOR else "Acreedor"
        return f"{self.tag} ({pos}, {self.amount:.2f} / {self.paid:.2f} / {self.remaining:.2f} €)"
...
class LoansHandler:
    ...
    def find(self, stop_date: datetime) -> list[Loan]:
        ...
        for event in self.data.events.subset(
            ...
            lambda x: re.match(LOAN_TAG_PATTERN, x.details),
        ):
            ...
            if tags:
                ...
                loans[tag].add(event.pullone())
        return list(sorted(loans.values()))

    def default(self, tag: str) -> None:
